...
export class BDPTPipeline extends Pipeline {
  ...
  init(context: GPUCanvasContext): void {
    ...
    const trianglesView = makeStructuredView(
      shaderDefs.storages.triangles,
      new ArrayBuffer(faces.length * trianglesSize)
    )
    ...
    const verticesView = makeStructuredView(
      shaderDefs.storages.vertices,
      new ArrayBuffer(vertices.length * verticesSize)
    )
    ...
    const materialsView = makeStructuredView(
      shaderDefs.storages.materials,
      new ArrayBuffer(materials.length * materialsSize)
    )
    ...
    this.bindGroups[3] = this.device.createBindGroup({
      ...
      entries: [{ binding: 0, resource: { buffer: imageBuffer } }],
    })
  }
  ...
  execute(frameNum: number): void {
    ...
    this.device.queue.writeBuffer(
      this.buffers.viewProjectionMatrixBuffer,
      0,
      new Float32Array(this.camera.inverseViewMatrix)
    )
...
export class RasterPipeline extends Pipeline {
  ...
  init(context: GPUCanvasContext): void {
    ...
    this.bindGroups[0] = this.device.createBindGroup({
      layout: this.pipeline.getBindGroupLayout(0),
      entries: [{ binding: 0, resource: { buffer: this.buffers.viewProjectionMatrixBuffer } }],
    })
  }
  ...
  execute(): void {
    ...
    this.device.queue.writeBuffer(
      this.buffers.viewProjectionMatrixBuffer,
      0,
      new Float32Array(mat4.transpose(mat4.create(), viewProjectionMatrix))
    )
