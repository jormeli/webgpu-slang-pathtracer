...
function readObj(obj: string, mtl?: string): [vec3[], Object3D[], Material[]] {
    ...
    const allVertices = data.models.map(m => m.vertices).flat().map(v => vec3.fromValues(v.x, v.y, v.z))
    ...
    return [
        allVertices,
        objects,
        [{ surfaceColor: [1, 1, 1] }, ...materials.map(m => ({ surfaceColor: [m.Kd.red, m.Kd.green, m.Kd.blue], emissionColor: [m.Ke.red, m.Ke.green, m.Ke.blue] }))]
    ]
}
