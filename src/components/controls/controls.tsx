...
const Label = (props: React.ComponentProps<'span'>) => {
  return (
    <span
      ...
      className={cn('align-middle text-sm text-muted-foreground text-left truncate', props.className)}
    />
  )
}
        ...
        <div className="px-2 w-full">
          <div className="grid grid-cols-[minmax(40px,_1fr)_210px] gap-2">
            ...
            <ToggleGroup
              ...
              onValueChange={(v) => {
                if (v) renderMode.value = v as RenderMode
              }}
            >
              <ToggleGroupItem value="raster" className="rounded-l-sm grow">
                Raster
              </ToggleGroupItem>
              <ToggleGroupItem value="trace" className="rounded-r-sm grow">
                Trace
              </ToggleGroupItem>
            </ToggleGroup>
            <Label>Max Samples</Label>
            <DraggableNumberInput
              value={maxSamples.value}
              step={1}
              onChange={(v) => (maxSamples.value = v)}
              className="justify-self-end w-full"
            />
          </div>
        </div>
