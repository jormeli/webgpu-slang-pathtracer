...
export const DraggableNumberInput = React.forwardRef<
  HTMLInputElement,
...
export default function VectorInput(props: {
  ...
  return (
    <div className={cn('flex flex-row gap-1', props.className)}>
      <DraggableNumberInput value={value[0]} label="x" onChange={(v) => onChange([v, value[1], value[2]])} />
      <DraggableNumberInput value={value[1]} label="y" onChange={(v) => onChange([value[0], v, value[2]])} />
      <DraggableNumberInput value={value[2]} label="z" onChange={(v) => onChange([value[0], value[1], v])} />
    </div>
  )
}
