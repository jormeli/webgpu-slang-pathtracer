...
export default function AxesHelper(props: { camera: Camera } & React.ComponentProps<'svg'>) {
    ...
    const [ viewProjectionMatrix, setViewProjectionMatrix ] = useState(mat4.multiply(mat4.create(), camera.projectionMatrix, camera.viewMatrix))
    ...
    const onAxisClick = useCallback((axis: number) => {
        ...
        newFwd[axis] = -1
        const target = mat4.lookAt(mat4.create(), pos, vec3.add(vec3.create(), pos, newFwd), axis === 1 ? [0, 0, 1] : [0, 1, 0])
    ...
    return (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox={`0 0 ${width} ${height}`} className={cn("w-[150px] h-[150px] select-none", className)} {...rest} onMouseDown={onMouseDown} onMouseEnter={() => circleRef.current?.style.setProperty('opacity', '1.0')} onMouseLeave={() => circleRef.current?.style.setProperty('opacity', '0.0')}>
            ...
            {sorted.map(([axis, idx]) => {
                return (
                    <g key={`axis-${idx}`} className="opacity-75 hover:opacity-100 cursor-pointer" onClick={() => onAxisClick(idx)}>
                        <line x1={width / 2} y1={height / 2} x2={width / 2 + axis[0]} y2={height / 2 - axis[1]} stroke={colors[idx]} strokeWidth="1" />
                        <circle cx={width / 2 + axis[0]} cy={height / 2 - axis[1]} r={circleRadius} fill={colors[idx]} />
                        <text x={width / 2 + axis[0]} y={height / 2 - axis[1]} textAnchor="middle" dy="0.3em" fill="white" fontSize="4">{['X', 'Y', 'Z'][idx]}</text>
                    </g>
                )
            })}
        </svg>
